% Starter code prepared by James Hays for Computer Vision

%This function will train a linear SVM for every category (i.e. one vs all)
%and then use the learned linear classifiers to predict the category of
%every test image. Every test feature will be evaluated with all 15 SVMs
%and the most confident SVM will "win". Confidence, or distance from the
%margin, is W*X + B where '*' is the inner product or dot product and W and
%B are the learned hyperplane parameters.

function predicted_categories = svm_classify(train_image_feats, train_labels, test_image_feats)
% image_feats is an N x d matrix, where d is the dimensionality of the
%  feature representation.
% train_labels is an N x 1 cell array, where each entry is a string
%  indicating the ground truth category for each training image.
% test_image_feats is an M x d matrix, where d is the dimensionality of the
%  feature representation. You can assume M = N unless you've modified the
%  starter code.
% predicted_categories is an M x 1 cell array, where each entry is a string
%  indicating the predicted category for each test image.

%{
Useful functions:
 matching_indices = strcmp(string, cell_array_of_strings)
 
  This can tell you which indices in train_labels match a particular
  category. This is useful for creating the binary labels for each SVM
  training task.

[W B] = vl_svmtrain(features, labels, LAMBDA)
  http://www.vlfeat.org/matlab/vl_svmtrain.html

  This function trains linear svms based on training examples, binary
  labels (-1 or 1), and LAMBDA which regularizes the linear classifier
  by encouraging W to be of small magnitude. LAMBDA is a very important
  parameter! You might need to experiment with a wide range of values for
  LAMBDA, e.g. 0.00001, 0.0001, 0.001, 0.01, 0.1, 1, 10.

  Matlab has a built in SVM, see 'help svmtrain', which is more general,
  but it obfuscates the learned SVM parameters in the case of the linear
  model. This makes it hard to compute "confidences" which are needed for
  one-vs-all classification.

%}

%unique() is used to get the category list from the observed training
%category list. 'categories' will not be in the same order as in proj4.m,
%because unique() sorts them. This shouldn't really matter, though.
M = size(test_image_feats, 1);
force_train = 1;
d = size(train_image_feats, 2);
categories = unique(train_labels); 
type_of_training = 1;   % 1 = vl_svmtrain(), 2 = primal_svm() with conjugate gradient, 3 = vl_svmtrain() with chi_2 kernel

if ~exist('W.mat') || ~exist('B.mat') || force_train==1
    num_categories = length(categories);
    lambda = 0.0001;
    W = zeros(d, num_categories);
    B = zeros(1, num_categories);
    cellfind = @(string)(@(cell_contents)(strcmp(string, cell_contents))); %inline function for comparing strings in cell array
    if type_of_training == 1
        train_image_feats = train_image_feats';
        train_labels = train_labels';
        for i=1:num_categories
            category = categories(i);
            labels = cellfun(cellfind(category), train_labels);
            labels = labels .* 2 -1;
            [w b] = vl_svmtrain(train_image_feats, labels, lambda);
            W(:, i) = w;
            B(i) = b;
        end

        save('W.mat', 'W');
        save('B.mat', 'B');
        fprintf('Saved W,B values');
        prediction_confidences = test_image_feats * W + repmat(B, size(test_image_feats, 1), 1);
    end
    if type_of_training == 2
        opt.cg = 1;
        for i=1:num_categories
            category = categories(i);
            labels = cellfun(cellfind(category), train_labels);
            labels = labels .* 2 -1;
            [w, b ]=primal_svm(train_image_feats, labels, lambda, opt); 
            W(:, i) = w;
            B(i) = b;
        end

        save('W.mat', 'W');
        save('B.mat', 'B');
        fprintf('Saved W,B values');
        prediction_confidences = test_image_feats * W + repmat(B, size(test_image_feats, 1), 1);
    end
    if type_of_training == 3
        train_image_feats = train_image_feats';
        train_labels = train_labels';
        test_image_feats = test_image_feats';
        hom.kernel = 'KChi2';   % use chi-squared kernel
        hom.order = 4;
        lambda = 0.0001;
        % create the dataset structure
        dataset = vl_svmdataset(train_image_feats, 'homkermap', hom);
        testset = vl_svmdataset(test_image_feats, 'homkermap', hom);
        prediction_confidences = zeros(M, num_categories);
        for i=1:num_categories
            category = categories(i);
            labels = cellfun(cellfind(category), train_labels);
            labels = labels .* 2 -1;
            [w b] = vl_svmtrain(dataset, labels, lambda);
            [~,~,~, scores] = vl_svmtrain(testset, labels, 0, 'model', w, 'bias', b, 'solver', 'none') ;
            prediction_confidences(:, i) = scores';
        end

        save('W.mat', 'W');
        save('B.mat', 'B');
        fprintf('Saved W,B values');
    end
else
    load('W.mat');
    load('B.mat');
    prediction_confidences = test_image_feats * W + repmat(B, size(test_image_feats, 1), 1);
end

[~, prediction_indices] = max(prediction_confidences, [], 2);
predicted_categories = categories(prediction_indices);
end