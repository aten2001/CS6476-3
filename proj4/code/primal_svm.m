function [sol,b,obj] = primal_svm(X,Y,lambda,opt)
...
% -----------------------------------------------------
  ...
  while 1
    ...
    % Do an exact line search
    [t,out] = line_search_linear(w,s,out,X, Y,lambda);
    w = w + t*s;
      
    % Compute the new gradient
    [obj, gn] = obj_fun_linear(w,X, Y,lambda,out); gn=-gn;
...
function y = hess_vect_mult(w,X, sv,lambda)
