function imdb = proj6_part3_setup_data(averageImage)
...
SceneJPGsPath = '../data/sun_db/';
load('../data/sun_db/attributes.mat');
load('../data/sun_db/images.mat');
load('../data/sun_db/attributeLabels_continuous.mat');

% we don't care about the category of the image in this case - we only care
% about the attributes a scene has - so the training/test splits can be
% done without worrying about the distribution of categories amongst the
% splits
...
zero_center = 1; % if 1, apply normalization

image_size = [224 224 3]; %downsampling data for speed and because it hurts
% accuracy surprisingly little

imdb.images.data   = zeros(image_size(1), image_size(2), image_size(3), total_images, 'single');
imdb.images.labels = zeros(102, total_images, 'single');
imdb.images.set    = zeros(1, total_images, 'uint8');

fprintf('Loading %d train and %d test images \n', ...
          num_train, num_test)
fprintf('Each image will be resized to %d by %d\n', image_size(1),image_size(2));

%Read each image and resize it to 224x224
for i = 1:total_images
        cur_path = fullfile(SceneJPGsPath, images{i});
        ...
        cur_image = imresize(cur_image, image_size(1:2));
        if (zero_center)
            for j=1:image_size(3)
                cur_image(:, :, j) = (cur_image(:, :, j) - mean2(cur_image(:, :, j)));
            end
        end
        % Stack images into a large 224 x 224 x 3 x total_images matrix
        % images.data
        imdb.images.data(:,:,:,i) = cur_image;
        imdb.images.labels(:, i) = labels_cv(i, :)';
        if i <= num_train
            imdb.images.set(1,i) = 1; %1 for train, 2 for test (val?)
        else
            imdb.images.set(1,i) = 2; %1 for train, 2 for test (val?)
        end
    end
end