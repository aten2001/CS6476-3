...
% run(fullfile('~/packages','matconvnet-1.0-beta16', 'matlab', 'vl_setupnn.m')) ;
...
opts.learningRate = logspace(-3, -4, opts.numEpochs);
...
jitter_on = 1; % if 1, randomly flip a fraction (jitter_factor) of the batch
jitter_factor = 0.6;
batchSize = size(batch, 2);
num_indices = batchSize*jitter_factor;
if (jitter_on)
    % original jittering
    im_indices = randi([1 batchSize], 1, num_indices);
    im(:, :, :, im_indices) = fliplr(im(:, :, :, im_indices));
    
    % extra jittering
    im_indices = randi([1 batchSize], 1, num_indices);
    % left-right mirroring
    im(:, :, :, im_indices(1:end/2)) = fliplr(im(:, :, :, im_indices(1:end/2)));
    % up-down mirroring
    im(:, :, :, im_indices(end/2:end)) = flipud(im(:, :, :, im_indices(end/2:end)));
end
