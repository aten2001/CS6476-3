...
function [bboxes, confidences, image_ids] = .... 
    run_detector_tree(test_scn_path, tree_model, feature_params)
...
for i = 1:length(test_scenes)
    ...
    scale_factor = 1;
    max_iterations = 1000; % max number of scaling down iterations in case of large images
    ...
    while i_min_dim*scale_factor > ts && iter < max_iterations % smallest dimension must at least be template size
        ...
        for j=1:ia-cz2+1
            for k=1:ib-cz2+1
                ...
                hog = reshape(hog, 1, []);
                v = predict_from_tree(tree_model, hog);
                if (v >= threshold)
                    cur_bboxes = [cur_bboxes; [k, j, k+cz2-1, j+cz2-1] * cz / scale_factor];
                    cur_confidences = [cur_confidences; v-threshold];
    ...
    % sort and pre-filter out low confidence bboxes
    pre_filter_on = 0;  % set to 1 to turn on pre_filtering
    max_num_bboxes = 5000; % maximum num of bboxes per image
    if pre_filter_on && size(cur_confidences, 1) > max_num_bboxes
        [~, indices] = sort(cur_confidences);
        indices = indices(1:max_num_bboxes);
        cur_bboxes = cur_bboxes(indices, :);
        cur_confidences = cur_confidences(indices);
        cur_image_ids = cur_image_ids(indices);
    end
