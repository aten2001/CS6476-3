...
if num_samples > 5000
    samples_per_image = samples_per_image + 20;
end
for i=1:num_images
    image = imread(fullfile(non_face_scn_path, image_files(i).name));
    image = im2single(rgb2gray(image));
    hogs = vl_hog(image, cz);
    [a, b, c] = size(hogs);
    for j=1:samples_per_image
        ix = randi(a);
        iy = randi(b);
        if(ix + cz2 < a) &&  (iy+cz2 <b)
            hog = hogs(ix:ix+cz2-1, iy:iy+cz2-1, 1:c);
            hog = reshape(hog, 1, []);
