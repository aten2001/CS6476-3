...
use_hard_negative = 0;
mine_hard_negative = 0;

tree_classifier_on = 0; % 0-use svm, 1-use random tree classifier
leaf_size = 1; % random tree leaf size
...
pos_count = size(features_pos, 1);
if use_hard_negative && exist('features_hard_neg.mat', 'file')
    load('features_hard_neg.mat');
    X = [features_pos; features_hard_neg];
else
...
if tree_classifier_on == 0
    ...
    tree_model = build_tree(X, Y, leaf_size);
end


%% step 2.5 further filter out further hard negative -- DISABLED
filtering_on = 0;
filter_threshold = 0.8;
if filtering_on && exist('features_hard_neg.mat', 'file')
    load('features_hard_neg.mat');
    v_values = features_hard_neg * w + repmat(b, size(features_hard_neg, 1), 1);
    filtered_hard_neg = features_hard_neg(v_values >= filter_threshold, :);
    save('filtered_hard_neg.mat', 'filtered_hard_neg');
end
...
if tree_classifier_on == 0
    ...
    if use_hard_negative
        confidences = [features_pos; features_hard_neg]*w + b;
        label_vector = [ones(size(features_pos,1),1); -1*ones(size(features_hard_neg,1),1)];
    else
    ...
    fprintf('Sanity check disabled for tree classifier');
end
...
if use_hard_negative == 0 && mine_hard_negative == 1
    features_hard_neg = run_detector_hard_negative(non_face_scn_path, w, b, feature_params);
...
if tree_classifier_on == 0
    ...
    [bboxes, confidences, image_ids] = run_detector_tree(test_scn_path, tree_model, feature_params);
end
