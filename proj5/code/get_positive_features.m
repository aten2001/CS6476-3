...
if mirrored_images == 0
    % without mirrored images
    ...
    for i=1:num_images
        ...
        features_pos(i, 1:D) = reshape(hog, 1, D);
    end
else
    % with mirrored images augmented
    ...
    for i=1:num_images
        ...
        hog = vl_hog(image, feature_params.hog_cell_size);
        features_pos(2*i-1, 1:D) = reshape(hog, 1, D);
        flipped_image = flip(image, 2);
        hog = vl_hog(flipped_image, feature_params.hog_cell_size);
        features_pos(2*i, :) = reshape(hog, 1, D);
    end
end