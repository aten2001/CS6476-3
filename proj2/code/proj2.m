...
%image1 = imread('../data/Pantheon Paris/1.jpg');
%image2 = imread('../data/Pantheon Paris/2.jpg');
...
max_no_points = 6000; % sort by confidence and limit the no. of points returned
[x1, y1, confidence_1] = get_interest_points(image1_bw, feature_width, max_no_points);
[x2, y2, confidence_2] = get_interest_points(image2_bw, feature_width, max_no_points);

 % Use cheat_interest_points only for development and debugging!
%[x1, y1, x2, y2] = cheat_interest_points(eval_file, scale_factor);

% Plot the corners detected
num_visualize = 500; % number of points to visualize
fig = figure(1);
subplot(1, 2, 1);
imagesc(image1_bw);
axis image;
colormap(gray);
hold on;
plot(x1(1:num_visualize), y1(1:num_visualize), 'rd');
...
colormap(gray);
hold on;
plot(x2(1:num_visualize), y2(1:num_visualize), 'rd');
title('Harris corners detected');

visualization_image = frame2im(getframe(fig));
imwrite(visualization_image, 'harris.jpg', 'quality', 100)
...
% !!! You will need to implement get_features. !!! DONE
